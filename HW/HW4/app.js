...
db.query("CREATE TABLE IF NOT EXISTS members (id INTEGER PRIMARY KEY AUTOINCREMENT, userId TEXT, password TEXT, category TEXT)");
...
router.get('/category/:category', categoryList); // 新增分版页面路由
...
function query(sql, params = []) {
    let list = [];
    for (const [id, userId, password, category] of db.query(sql, params)) {
        list.push({ id, userId, password, category });
    }
    return list;
}
...
async function build(ctx) {
    ...
    if (body.type === "form") {
        ...
        if (su == 3) { // 确保有 userId, password 和 category
            db.query("INSERT INTO members (userId, password, category) VALUES (?, ?, ?)", [member.userId, member.password, member.category]);
            console.log('create successful:', member);
...
async function check(ctx) {
    ...
    if (body.type === "form") {
        ...
        for (const [id, userId, password, category] of db.query("SELECT id, userId, password, category FROM members")) {
            let ct = 0;
            ...
            if (ct == 2) {
                ...
                LoginId = id;
                LOID = member.userId;
                ctx.response.redirect(`/category/${category}`); // 根据用户选择的版块跳转
            }
        }
        if (!cct) {
            console.log('fail');
...
async function categoryList(ctx) {
    ...
    ctx.response.body = await render.category(posts, category);
}
